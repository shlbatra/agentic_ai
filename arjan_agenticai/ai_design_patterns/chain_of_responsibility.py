...
# Information agents need and filled in step by step
# The ctx (context) in this file is a shared TripContext object that gets mutated in-place as each handler in the chain executes
...
# Functions executed in sequence, each updating the TripContext via ctx
#  Each handler:                                                                                                                                                                                                     
#   1. Reads from ctx to build its prompt using previously gathered info                                                                                                                                              
#   2. Writes its results back to ctx for downstream handlers                                                                                                                                                         
                                                                                                                                                                                                                    
#   The Flow:                                                                                                                                                                                                         
#   ┌────────────────────┬───────────────────────────────────────────────────────────────────────┬────────────────────────────────────┐                                                                               
#   │        Step        │                            Reads from ctx                             │           Writes to ctx            │                                                                               
#   ├────────────────────┼───────────────────────────────────────────────────────────────────────┼────────────────────────────────────┤                                                                               
#   │ handle_destination │ nothing                                                               │ ctx.destination                    │                                                                               
#   ├────────────────────┼───────────────────────────────────────────────────────────────────────┼────────────────────────────────────┤                                                                               
#   │ handle_flight      │ ctx.destination (line 126)                                            │ ctx.from_city, ctx.arrival_time    │                                                                               
#   ├────────────────────┼───────────────────────────────────────────────────────────────────────┼────────────────────────────────────┤                                                                               
#   │ handle_hotel       │ ctx.destination, ctx.arrival_time (lines 138-139)                     │ ctx.hotel_name, ctx.hotel_location │                                                                               
#   ├────────────────────┼───────────────────────────────────────────────────────────────────────┼────────────────────────────────────┤                                                                               
#   │ handle_activities  │ ctx.destination, ctx.hotel_location, ctx.arrival_time (lines 152-153) │ nothing (final step) 
#   └────────────────────┴───────────────────────────────────────────────────────────────────────┴────────────────────────────────────┘

async def handle_destination(
    user_input: str, deps: TravelDeps, ctx: TripContext # same arguments for all handlers and context filled in step by step
) -> None:
    dest_result = await destination_agent.run(user_input, deps=deps)
    ctx.destination = dest_result.output.destination # update context with each method
...
async def plan_trip(user_input: str, deps: TravelDeps):
    ...
    ctx = TripContext()
    # Easy to scale up handlers by adding here, make dynamic if needed based on user input within chain, also handle errors in chain or graph etc.
    chain = [
        handle_destination,
        handle_flight,
        handle_hotel,
...
async def main():
    ...
    await plan_trip("I want a sunny city trip within Spain.", deps)
