...
async def run_travel_strategy(
    user_prompt: str,
    deps: TravelDeps,
    strategy_fn: Callable[[], Agent[TravelDeps, TravelRecommendation]], # type of strategy function and returns an Agent
):
    agent = strategy_fn() # create agent here
...
async def main():
    deps = TravelDeps(user_name="Sahil")

    print("=== Professional Agent ===")
    await run_travel_strategy(
        "I want a peaceful place by the sea.", deps, professional_agent # pass the type of agent
    )
